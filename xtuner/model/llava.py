...
class LLaVAModel(BaseModel):
    ...
    def __init__(self,
        ...
        if pretrained_pth is not None: # load the pretrained checkpoint
            pretrained_state_dict = guess_load_checkpoint(pretrained_pth) # need a lot of time
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        organ_name = data.pop('organ_name', None)         # <-- MODIFIED from organ_type
        case_name = data.pop('case_name', None)           # <-- MODIFIED from slide_id
        question = data.pop('question', None)             # <-- ADDED
        answer = data.pop('answer', None)                 # <-- ADDED
        ...
        if 'pixel_values' in data:
            ...
            data['pixel_values'] = pixel_values # shape: [1, patch_num, 3584]
